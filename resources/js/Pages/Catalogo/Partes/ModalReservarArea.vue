<script setup>
	...
	watch(() => props.open, (val)=>{
		if (val) {
			...
			cargando.value = true
			reservas.value = [];
			axios.get(`/api/reservas/area/${area.value.id}`)
				.then(({ data }) => {
					// Serealizando
		...
		// Invalid
		area.value.fotos = [
			{id:3,ruta_imagen: 'https://raw.githubusercontent.com/vueComponent/ant-design-vue/main/components/carousel/demo/abstract04.jpg'},
			{id:30,ruta_imagen: 'https://raw.githubusercontent.com/vueComponent/ant-design-vue/main/components/carousel/demo/abstract02.jpg'}
		]
		
		if (area.value.fotos.length < 1) {
			area.value.fotos = [
		...
		<div class="flex w-auto  flex-wrap sm:flex-nowrap sm:justify-end gap-3">
			...
			<div>
				<Carousel :arrows="true" class="w-44 rounded-lg overflow-hidden self-start">
					...
					<img
						v-if="area.fotos && area.fotos.length > 0"
						v-for="foto of area.fotos"
				...
				<div class="pb-1 text-gray-500 pt-2">
					<div class="flex items-center gap-2">
						...
						{{area?.laboratorio?.nombre ?? 'Sin definir'}}
					</div>
				</div>
			</div>
			<div class="min-w-96">
				<h3 class="text-2xl font-bold">{{area.nombre}}</h3>
